...
DigitalIn::DigitalIn(PinName pin, PullType pull) : pin_(pin) {
    if (GpioClockInit(pin) != HAL_OK)
    {
        Error_Handler();
    }
...
void DigitalIn::DeInit() {
    if (HAL_GPIO_DeInit(pin_.port, pin_.pin) != HAL_OK)
    {
        Error_Handler();
    }
}
...
void DigitalIn::setPull(PullType pull) {
    ...
    if (HAL_GPIO_Init(pin_.port, &pin_structure) != HAL_OK)
    {
        Error_Handler();
    }
}